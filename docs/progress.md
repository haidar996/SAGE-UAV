...
## Option 2 (flag possible duplicates) tried offline and NOT shipped (2026-09-25)
- Rule: flag a verified entry as `possible_duplicate_of` an earlier one if d in [2.5, min(8, 2.5+1.2*dt)] and either read MOVING. Replayed on the TARGET VERIFIED lines of the 5 sage_hard runs 020852..040416: flags 1-4 per run, mostly REAL people (S2, S3, W1 flagged as duplicates of walkers/statics; W2's first reading even flagged as a duplicate of S1). Reason: verifications are tens of seconds apart, so the reach radius hits the 8 m cap for every pair; time gives no discrimination and only the MOVING flag remains, which is unreliable.
- Planner change reverted. scripts/score_trial.py now understands optional `?dup#N` markers and adds 3 columns (flagged, fp_if_flagged_dropped, fn_if_flagged_dropped); harmless without markers (old rows have 12 columns, header not updated).
- Useful variant (not built): store the walker's velocity DIRECTION at verification and flag only if the other reading lies near that heading line (W2 walks along x=-5; S3 is 3 m and S2 4 m off the walker lines). Needs velocity vectors logged first; direction noise (maybe 20-30 deg) is untested. Alternative: active disambiguation (option 1).

## Session 2026-09-25 (overnight, autonomous until 08:00) - what was done and decided
Context: the user asked for a last chance for the hard world (simplified to ONE walker), then sage_sar-style experiments, improvements to the environment, a demo video, pictures, a LinkedIn post file, and a full write-up by 08:00.

Findings and changes (all committed):
- Walker duplicates in sage_hard cannot be fixed by speed/distance rules (see design_identity_check.md, solutions_plan.md). Longer walker paths (sage_hard_long) helped visibility of motion (walker windows flagged moving 32 % -> 53 %, static 1 %) but not the duplicates; both long-world runs timed out at 900 s with W1 never swept.
- Candidate handling: 27/88 rejected candidates were re-tracks of verified people; an "early drop" made runs 445 s but LOST people (S3 dragged onto walker W1 by the refine step and blacklisted) -> replaced by a refine guard (a candidate is not refined onto a detection on an already verified person). mission_timeout_s 900 -> 1200. 25 s candidate timeout kept (real verifications take up to 20 s after arrival).
- Reliability: UAV-lost guard in the planner (outside area+6 m or on the ground for 8 s -> mission ends with the partial report, status `uav_lost`), PX4 offboard-loss failsafe = Hold (COM_OF_LOSS_T 2, COM_OBL_RC_ACT 5), stack start retries 3 -> 5. One 17 s sim stall + flyaway seen in 0925_024917 (cause unproven). ~7 % of stack starts flip/land far away; ~37 % of start attempts fail the health gate (mostly RTF < 0.5).
- New world `sage_rescue` (worlds/make_worlds.py): visually rich (roads, grass, helipad, houses, trees, bushes, cars, container; all props visual-only and low) with 3 static people + ONE walker on a 12 m path >= 5.5 m from every static person.
- Demo tooling: scripts/record_demo.py (onboard camera + YOLO boxes synced to the exact frame, top-down mission map, stage bar, battery, end card -> mp4 H.264 + stills), scripts/make_collage.py, scripts/make_figures.py (architecture, results charts, LinkedIn card), SAGE_RECORD=1 hook in scripts/run_trials.sh.
