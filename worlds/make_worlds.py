...
def build_rescue(world='sage_rescue'):
    ...
    for name, n, e, sn, se, h, rgb in [('car_red', 1.0, 9.0, 1.8, 4.2, 1.3, '0.8 0.1 0.1'),
        ...
        body += box(name, n, e, sn, se, h, rgb, False)
    # fixed overhead camera for the demo video: 30 m above the origin, looking straight down,
    ...
    <model name="overview_cam"><static>true</static><pose>0 0 30 0 1.5708 1.5708</pose>
      <link name="link"><sensor name="cam" type="camera"><always_on>1</always_on><update_rate>5</update_rate>
        <topic>overview_cam</topic>
        <camera><horizontal_fov>0.95</horizontal_fov><image><width>800</width><height>800</height></image>
