...
for attempt in 1 2 3; do
  ...
  for i in $(seq 1 60); do grep -q "stack-up" $S/stack.out 2>/dev/null && break; sleep 5; done
  if ! grep -q "stack-up" $S/stack.out; then echo "attempt $attempt: stack did not come up"; continue; fi
  sleep 15
